...
const getStyle = (element:HTMLElement,property:string):string=>{
    ...
    if(windowObj){
      const getComputedStyle:Function = windowObj.getComputedStyle;
...
//3, 知道元素当前的初始化位置
const getTargetPos = (element:HTMLElement):Pos=>{
    let pos = {x:0,y:0};
    //判断当前浏览器是否支持transform
    const transform:any = checkTransform();
    if(transform){
        //获取transform样式，返回一个2D变化矩阵
        const transformValue:string = getStyle(element,transform);
...
//4，设置新的css样式达到移动的效果
const setTargetPos = (element:HTMLElement,pos:Pos):void=>{
    //获取支持情况
    const transform:any = checkTransform();
    if(transform){
...
//获取目标元素
const moveBox = <HTMLDivElement>document.getElementById("target");
// 设置鼠标的初始位置
let mouseX = 0;
let mouseY = 0;
//设置元素的初始位置
...
//鼠标点击的时候，做的工作
const start = (event:MouseEvent):any=>{
    //获取鼠标的初始位置
    mouseX = event.pageX;
    mouseY = event.pageY;
    //获取元素的初始位置
